...
def load_dataset(task_name: str,
    ...
    if split_valid:
        ...
        # 2-way split matching the reference code (no validation set)
        train_df, test_df = train_test_split(dataset.raw_data, test_size=test_size, random_state=seed)
        dataset = {
...
def rank_of_token(last_logits: torch.Tensor, token_id: int) -> int:
    target_logit = last_logits[0, token_id]
    return 1 + (last_logits[0] > target_logit).sum().item()

def topk_acc(ranks, K: int) -> float:
    if not ranks:
        return 0.0
    t = torch.tensor(ranks)
    return float((t <= K).float().mean().item())


def compute_filter_set(model, tokenizer, dataset, split, n_icl_examples,
                       prefixes, separators, device, seed=42):
    """Evaluate n-shot ICL on every example in `split` and return indices
    where the model ranks the correct token first (rank == 1).

    This mirrors the Function Vectors paper's per-example filtering:
    only examples the model already solves via ICL are used as query
    positions for mean-activation collection and AIE computation.

    Parameters:
        model: the language model
        tokenizer: huggingface tokenizer
        dataset: dict with 'train' and the target split (e.g. 'valid', 'test')
        split: which split to evaluate ('valid' or 'test')
        n_icl_examples: number of ICL demonstration pairs
        prefixes: ICL template prefixes
        separators: ICL template separators
        device: torch device
        seed: random seed for sampling ICL examples

    Returns:
        filter_set: np.ndarray of indices where rank == 1
        ranks: list of ranks for every example in the split
    """
    model_config_prepend_bos = True
    prepend_bos = False  # model already prepends BOS
    ...
    with torch.inference_mode():
        for j in range(len(split_data)):
            train_perm = torch.randperm(
                len(dataset['train']), generator=generator
            )[:n_icl_examples].tolist()
            word_pairs = dataset['train'][train_perm]

            word_pair_test = split_data[j]
    ...
    filter_set = np.where(np.array(ranks) == 1)[0]
    return filter_set, ranks